...
以上操作需要载入模块`Data.Ratio`，执行命令是`:module Data.Ratio`。
这里，可以用 ``:m``这个缩写 来替代 ``:module``。``:m``跟着模块名，表示用于载入一个给定模块，若不给模块名，则是卸载模块。Ghci
还提供了很多这类缩写，方便使用者。
...
2. 在\ **ghci**\ 里输入\ **:?**\ 以获取帮助信息。定义一个变量，比如\ ``let x = 1``,然后输入\ ``:show bindings``.你看到了什么？
