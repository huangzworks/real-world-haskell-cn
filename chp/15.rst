...
一个简单有效的解决办法是为 ``Supply`` 提供一个更好的随机数源，但现在我们把这个方法放到一边，考虑一种在许多设定中都有效的替代方法。我们将使用一个类型类把我们可以用monad执行的动作和该动作具体如何实现分离。
...
现在让我们回头看之前忽略的代码段， ``| m -> s`` 是一个功能依赖，通常被称作 ``fundep`` 。我们可以将竖线 ``|`` 读作 “这样”，将箭头 ``->`` 读作“唯一确定”。我们的功能依赖建立了 ``m`` 和 ``s`` 之间的关系。

功能依赖由 ``FunctionalDependencies`` 语言编译指令管理。

我们声明 ``m`` 和 ``s`` 之间关系背后的目的是帮助类型检查器。回想一下，Haskell类型检查器本质上是一个定理证明器，并且它在操作上是保守的：它坚持这个证明过程必须终止。一个非终止的证明结果将导致编译器放弃或陷入无限循环。
...
假如我们没有功能依赖，类型检查器不会发现 ``MonadSupply s`` 和 ``Supply s`` 二者类型参数之间的关系，因此它将终止编译并产生一个错误。这个定义本身将会编译，而类型错误直到我们尝试使用它的时候才会产生。
