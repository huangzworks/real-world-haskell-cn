...
``State`` monad 让我们通过代码传递了一些可变状态。有时，我们希望能够传递一些不可变的状态，比如程序的配置数据。在这种情况下我们仍可以使用 ``State`` monad ，但我们可能会发现自己不小心修改了不可变数据。
...
我们可以将类型 ``e`` 表示的值理解为计算某个表达式所处的环境。因为 ``return`` 动作应当在任何情况下具有相同的效果，所以我们的代码忽视了它所处的环境。

我们定义的 ``(>>=)`` 有点复杂，但这仅仅是因为我们要让环境（代码中是变量 ``r`` ）在当前和接下来的计算中能够使用。

在 monad 中执行的一段代码如何知道它的环境中有什么？这通过 ``ask`` 来实现。
...
``Reader`` monad 包含在标准 ``mtl`` 库中，它通常与 GHC 捆绑在一起。你可以在 ``Control.Monad.Reader`` 模块中找到它。你可能不明白设计这个 monad 的动机是什么，实际上它通常在复杂的代码中更有用。我们时常需要访问程序深处的一段配置信息，若将这些信息作为正常参数传递，我们需要对代码做一次极为痛苦的重构。如果将这些信息隐藏在我们 monad 的管道中，那么不关心配置信息的中间函数就不需要看到它们。

使用 ``Reader`` monad 最清晰的动机将在 :ref:`monad-transformers` 中介绍，我们将讨论如何把几个 monad 合并为一个新的 monad 。那时候，我们会看到如何利用 ``Reader`` monad 更好地控制状态，这样我们的代码可以通过 ``State`` monad 修改一些值，而其他值保持不变。
...
注意到我们的类型必须是 ``MonadSupply e`` 的实例，而不是 ``MonadSupply`` 。如果我们省略了类型变量，编译器将会报错。[译注：在类型声明中指定类型依赖的方式是在类名和实例名中使用相同的参数名。]
...
因为我们的 ``MySupply`` monad 由两层 ``newtype`` 包装，我们可以为它编写一个自定义的执行函数使其更易使用。
...
``IO`` monad 的祝福和诅咒都来源于它的过分强大。假如我们相信小心地使用类型能帮助我们杜绝程序错误，那么 ``IO`` monad 将成为错误的一个重要来源。因为 ``IO`` monad 对我们做的事情不加以限制，这使我们容易遭受各种意外。
...
我们要创建有限制 ``IO`` 的第一步是将其用 ``newtype`` 包装。
...
剩下的工作就是包装我们希望 monad 允许的每个动作。使用 ``HandleIO`` 数据构造函数包装每个 ``IO`` 可以很简单地实现。
...
3. 在表单编码的字符串中，相同的键可能出现数次，具有获不具有值，例如 ``key&key=1&key=2`` 。你会用什么类型来表示这类字符串中和键相关联的值？编写一个正确捕获所有信息的解析器。
