...
设计这个 monad 的目的是让我们更容易在普通情况下编写无错代码，而非不允许特殊情况。让我们给自己留一条出路。
...
我们对这个类型类的实现是非常简单的：仅仅用自己的数据构造器将 ``IO`` 包装。
...
类型类方法的美丽在于，我们可以不需要接触过多代码并互换一个底层的 monad ，因为大多数代码不知道或不关心具体底层 monad 的实现。例如，可以用一个 monad 来替换 ``IO`` ，它会在文件写入时压缩文件。

通过类型类定义一个 monad 的接口由另一个好处。它允许其他人在 ``newtype`` 包装器中隐藏我们的实现，并自动派生他们想要暴露的类型类的实例。
...
事实上，因为 ``safeHello`` 函数没有使用 ``IO`` 类型，我们甚至可以使用不能执行 I/O 的 monad。这允许我们测试那些在完全纯粹的受控环境中会有副作用的代码。

为此，我们将创建一个不执行 I/O 的 monad，但记录每个和文件相关的事件以供后续处理。
...
虽然在 :ref:`using-a-new-monad-show-your-work` 中已经开发了一个 ``Logger`` 类型，但这里我们将使用标准的、更广泛的 ``Writer`` monad。和其它 ``mtl`` monad 类似， ``Writer`` 提供的 API 在一个类型类中被定义，这里是 ``MonadWriter`` 。它最有用的方法是 ``tell`` ，它记录了一个值。
...
记录的值可以是任意 ``Monad`` 类型。因为列表类型是 ``Monoid`` ，因此我们要记录到一个 ``Event`` 列表。

我们可以让 ``Writer [Event]`` 成为 ``MonadHandle`` 的一个实例，但编写一个特殊用途的 monad 更容易也更安全。
...
执行函数简单的移除了之前添加的 ``newtype`` 包装器，之后调用了普通 ``Writer`` monad 的执行函数。
...
当我们在 **ghci** 中尝试时，它反馈给我们这个函数与文件有关的活动记录。
...
``Writer`` monad 在每次使用 ``tell`` 时使用 monoid 的 ``mappend`` 函数。因为列表的 ``mappend`` 是 ``(++)`` ，所以在实际应用中，列表不是一个很好的选择：重复附加操作是昂贵的。我们使用列表纯粹是为了简单。

在生产代码中，假如你想使用 ``Writer`` monad 并且需要类似列表的行为，你应当使用一个具有更好的附加功能的类型。一个这样的类型是 :ref: `taking-advantage-of-functions-as-data` 中介绍过的差异列表。你不需要实现自己的差异列表：可以从 Haskell 包数据库 Hackage 下载调试好的库。或者，你可以使用 ``Data.Sequence`` 模块中的 ``Seq`` 类型，我们在 :ref:`general-purpose-sequences` 中介绍过。
...
如果使用类型类方法来限制 ``IO`` ，我们可能仍然希望保留执行任意 I/O 操作的能力。因此可以尝试添加 ``MonadIO`` 作为类型类约束。
...
但是这种方法有一个问题：添加的 ``MonadIO`` 约束失去了在纯环境中测试代码的能力，因为我们不能再判断一个测试是否可能有有害的的副作用。另一种方法是在类型类中移动这个约束，使它只影响那些真正需要执行 I/O 操作的函数，而不是影响所有函数。
...
我们可以对不受 ``MonadIO`` 约束的函数使用纯属性测试，对其余的使用传统的单元测试。

不幸的是，我们将一个问题替换成了另一个问题：现在不能通过只具有 ``MonadHandle`` 约束的代码来调用同时具有 ``MonadIO`` 和 ``MonadHandle`` 约束的代码。如果只有 ``MonadHandle`` 约束的代码真的需要 ``MonadIO`` 约束，我们就必须将这个约束添加到所有导致这一点的代码中。

允许任意 I/O 是有风险的，并且对我们如何开发、测试代码有深刻影响。当必须在更宽容的限制和更容易的推理、测试间做选择时，我们通常选择后者。
