...
每当我们使用 ``tell`` 方法， ``Writer`` monad 就会调用 monoid 的 ``mappend`` 函数。因为列表的 ``mappend`` 动作是 ``(++)`` ，而不断重复附加操作是非常浪费资源的，所以在实际使用 ``Writer`` 时，列表不是一个很好的选择。上面的示例中，我们纯粹是为了追求简单才使用了列表。

在生产代码中，如果你想使用 ``Writer`` monad ，并且需要类似列表的行为，那么你应当选择那些在执行附加操作时性能更优的类型。在 :ref:`taking-advantage-of-functions-as-data` 中介绍过的差异列表就是一个这样的类型。你不需要实现自己的差异列表：Haskell 包数据库 Hackage 提供了一个调试好的库，你可以直接下载使用。或者，你可以使用 ``Data.Sequence`` 模块中的 ``Seq`` 类型，我们在 :ref:`general-purpose-sequences` 中介绍过这个类型。
...
如果使用类型类方法来限制 ``IO`` ，我们可能仍然希望保留执行任意 I/O 操作的能力。因此，我们可以尝试将 ``MonadIO`` 作为约束添加到类型类中。
...
但是这种方法存在问题：因为添加的 ``MonadIO`` 约束使我们无法再判断一个测试是否可能导致副作用，所以我们失去了在纯粹环境中测试代码的能力。另一种方法是调整这个约束的作用域，使它只影响那些真正需要执行 I/O 操作的函数，而非所有函数。
...
不幸的是，我们仅仅是将一个问题替换成了另一个问题：只受 ``MonadHandle`` 约束的代码将无法调用同时受 ``MonadIO`` 和 ``MonadHandle`` 约束的代码。如果某处只受 ``MonadHandle`` 约束的代码确实依赖一些受 ``MonadIO`` 约束的代码，我们就必须将 ``MonadIO`` 约束添加到所有导致这一点的代码中。
