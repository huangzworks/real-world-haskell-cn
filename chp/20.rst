...
现在来试试这些新特性。首先确定一下之前例子种的命令是否还能工作。然后，使用新的类 shell 语法运行一下。
...
本章中，我们从零开始开发了一个精简版的 ``HSH`` 。如果你希望使程序具有这种类 shell 的功能，我们推荐使用 ``HSH`` 而非上面开发的例子，因为 ``HSH`` 的实现更加优化。``HSH`` 还有一个数量庞大的工具函数集和更多功能，但其背后的代码也更加庞大和复杂。其实例子中很多工具函数的代码我们是直接从 ``HSH`` 抄过来的。可以从 http://software.complete.org/hsh访问 ``HSH`` 的源码。
