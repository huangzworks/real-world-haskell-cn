...
Everything from file timestamps to business transactions involve dates and times. Haskell provides ways for manipulating dates and times, as well as features for obtaining date and time information from the system. No comments

ClockTime and CalendarTime
^^^^^^^^^^^^^^^^^

In Haskell, the System.Time module is primarily responsible for date and time handling. It defines two types: ClockTime and CalendarTime. 2 comments
