...
我们可以用 Haskell 轻松构建一个简单而实用的 “山寨版” MapReduce。我们将把重点落在 Web 服务器日志文件的处理上，这些文件通常规模较大且信息丰富 [57]_ 。

下面这个例子是 Apache Web 服务器记录的一条访问记录。这条记录原本是一行，这里为了阅读方便将其切分成多行。
...
* 算法很快被切分和通信过程的细节所掩盖，这会导致代码难以理解，从而使修改代码的风险加大；
* 选择 “粒度” —— 很难确定计算计算核心处理的最小工作单元。如果粒度太小，核心将会花费大量的时间保存上下文，从而使并行程序比串行更慢。如果粒度太大，那么有些核心受限于负载均衡的效果，可能会闲置。
