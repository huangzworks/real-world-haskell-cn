...
GHC除了可以进行基本的时间和空间分析外，还能为程序整个执行期间的堆内存使用
...
构建内存使用情况的性能调试和构建一般时间占用调试的步骤是一样的。
...
一个内存使用的性能调试的log文件 ``A.hp`` 被会创建，其内容为以下形式：
...
我们能从图片中看出什么？第一点，程序执行分两个阶段：
在计算总和的同时， 前一阶段不断分配大量的内存，后一半清理释放这些内存。
