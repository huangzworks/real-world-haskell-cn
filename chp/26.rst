...
至此我们已经创建了一个不算太复杂的库，它包括四个公共模块和一个内部模块。现在让我们来创建一个 ``rwh-bloomfilter.cabal`` 文件，将这个库打包成容易发布的格式。

Cabal 允许我们在一个包中描述几个库的信息。 ``.cabal`` 文件以所有库通用的信息开头，后面跟着各个库不同的部分。
...
Cabal 包需要指明自己构建时依赖的其它包。这些信息帮助 Cabal 的命令行接口在必要的情况下自动下载并构建包的依赖。我们希望编写的代码能够尽量兼容更多的 GHC 版本，不管这些版本是否具备 ``base`` 和许多其它包的现代布局。举个例子，我们的代码要能够在 ``array`` 包存在的时候说明自己依赖它，否则就只能依赖 ``base`` 包。
...
* 配置功能在 Cabal 的 1.2 版本中引入，因此上面指定 Cabal 版本不能低于 1.2。
* ``split-base`` 标识的含义不言而喻。[Forec 译注：该标识表示 ``base`` 包是否被划分]
* ``bytestring-in-base`` 标识处理一段更为曲折的历史：``bytestring`` 包在创建之初是和 GHC 6.4 捆绑的，并且它始终独立于 ``base`` 包；在 GHC 6.6 中，它被合并到了 ``base`` 包中；到了 GHC 6.8.1 版本，它又再次被独立出去。
...
..info::
	...
	在编写 FFI 绑定时，``-fvia-C`` 编译选项能够在安全性上带来提升。如果在 FFI 声明中提到头文件（如外部导入 ``string.h memcpy``），那么 C 编译器会对生成的 Haskell 代码做类型检查，以确保其 C 函数的调用和头文件中的函数原型一致。
	
	如果不使用 ``-fvia-C`` ，我们将失去一层额外的安全保障。这将使 C 类型错误很容易混入我们的 Haskell 代码中。举个例子，在多数 64 位机器上，``CInt`` 是 32 位宽，而 ``CSize`` 是 64 位宽。如果我们不小心弄混了用来描述 FFI 绑定参数的类型，那么数据可能会损毁。
...
普通的 ``quickCheck`` 函数对布隆过滤器属性的测试帮助不大，因为它产生的 100 个测试输入样例无法完整覆盖布隆过滤器的功能。因此我们编写自己的测试函数：
...
* 通过结构：我们制定要生成的有效值的范围。QuickCheck 为此提供了 ``forAll`` 组合器。
