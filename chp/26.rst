...
Haskell 提供了一个特殊的 Monad，叫做 ``ST`` [58]_ *（State Transformer）* 。 ``ST`` 允许我们安全地工作在可变状态下。与 ``State`` Monad 相比，它有一些强大的额外的功能。
...
让我们重新构建这个程序，并在启用分析的情况下运行它，以观察哪些调优能够改善它的性能。

因为此前我们已经构建了 ``WordTest`` 并且没有做任何改动，如果仅仅重新运行 GHC，GHC 会检测到已存在的二进制文件，因此它并不会重新编译。我们必须强制重新构建此程序，这里可以通过编辑源文件以更新文件系统来实现。

[Forec 译注：也可以使用 ``=fforce-recomp`` 标识来实现，它会强迫 GHC 重新构建。]
...
回忆一下，``doubleHash`` 函数的主体是一个无副作用的列表分析：
...
鉴于 ``doubleHash`` 返回一个列表，它占这么大内存似乎有一定理由。但是这么简单的代码却表现出如此之差的性能，不由得让人怀疑。

面对这么一个性能谜团，我们自然会想到检查编译器的输出。我们不需要通过汇编语言转储来分析：从更高层次开始会更好。

GHC 的 ``-ddump-simpl`` 选项会打印出执行了所有高级优化后生成的代码：
...
产生的 ``dump.txt`` 大约有一千行，其中多数名字是依据原始的 Haskell 代码表示生成的。即使如此，搜索 ``doubleHash`` 可以帮助我们立刻定位到函数的定义。下面这个例子说明了如何在 Unix Shell 中寻找到函数的定义：
...
* 这个模式将在 Haskell 中读作 ``(y_a1DW:ys_a1DX)`` 。``(:)`` 构造器之所以出现在操作数之前，是因为 Core 语言出于简单起见使用了前缀表达式；
...
* 这是 ``(:)`` 构造器的第二个参数，其中 ``go_s1YC`` 函数以递归的方式应用自身；
* 这里应用了列表解析函数。它的参数是用 Core 语言表示的 ``[0..n]`` 。
...
为了解决这些问题，我们对 ``doubleHash`` 函数做了一点微小的工作：
...
上面的代码中，我们手动将 ``[0..num]`` 表达式和 “消费” 它的代码合并成单个循环，并为 ``h1`` 和 ``h2`` 添加了严格注释。这些改动将 6 行代码变成 8 行，除此之外没有任何其它改动。它们会对 Core 的输出有什么影响呢？
...
* ``easyList`` 中使用的 ``genericLength`` 在处理无限列表时会导致函数陷入无限循环。如何修复？
...
.. [59] Jenkins 的哈希函数相比一些流行的非加密哈希函数（如 FNV 和 hashpjw）具有好的多的混合属性，因此我们建议避免使用那些非加密哈希函数。
