...
列表这种常见的类型就是\ *递归*\的：即它用自己来定义自己。为了深入了解其中的含义，让我们自己来设计一个类列表类型。我们将用 `Cons` 替换 `(:)` 构造器，用 `Nil` 替换 `[]` 构造器。
...
`List a` 在 `=` 符号的左右两侧都有出现，我们可以说该类型的定义引用了它自己。当我们使用 `Cons` 构造器来创建一个值时，我们必须提供一个 `a` 的值作为参数一，以及一个 `List a` 类型的值作为参数二。接下来我们看一个实例。
...
稍有不同的是，Java 中使用特殊值 `null` 表示各种“没有值”, 因此我们可以使用 `null` 来表示一个节点没有左子节点或没有右子节点。下面这个简单的函数能够构建一个有两个叶节点（叶节点这个词习惯上是指没有子节点的节点）的树。
...
2. 仿造 Java 示例里那样，定义一种只需要一个构造器的树类型。不要使用 `Empty` 构造器，而是用 `Maybe` 表示节点的子节点。
