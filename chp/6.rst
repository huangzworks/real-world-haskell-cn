...
请注意，存在着更多数值类型用于特定的目的，比如提供接口给 ``C`` 。
...
我们现在还不用关心实现细节，所以让我们暂时使用 ``undefined`` 作为函数体。
...
.. note:: 什么时候重叠实例很重要(matter)？

    就像我们之前提到的，我们可以把一个类型类的实例分散到各个模块中。
    GHC 不会抱怨重叠实例的单独存在(mere existence)。
    取而代之地，只有当我们试图使用受影响的类型类的函数时，只有他被迫要去做决定采用哪个实例时，它才会抱怨。
