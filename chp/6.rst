...
下面的例子没有使用 ``D`` 构造器，因而未被保护的 ``undefined`` 会被求值，当模式匹配发生时，然后我们抛出异常。
...
当我们用 ``N`` 构造器创建 ``NewtypeInt`` 值时，我们能看到相同的行为：没有异常，相比于使用 ``DataInt`` 类型的 ``D`` 构造器。
...
决定性的（crucial）不同点发生了，当我们从表达式中去掉 ``N`` ，并对未保护的 ``undefined`` 进行模式匹配时。
