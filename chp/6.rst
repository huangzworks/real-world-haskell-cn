...
.. _note:
    ...
    当我们定义一个实例，其（这个实例）我们希望能（被）允许覆盖（overlap）于其他实例的时候，我们必须激活(enable，使能)该扩展(``OverlappingInstances``)为这个模块，而其（这个模块）包含着定义。当他编译这个模块的时候，GHC会记录那个实例为＂能被覆盖（overlap）以其他的模块＂。
...
至少有时，因而，我们就能避免对 ``OverlappingInstances`` (覆盖同例)扩展的需求，带着一点点(时间维度的)横向思维(lateral thinking)。

[sancao2译注:lateral thinking, 字面译成后向的思维，就是这里的说的在"覆盖同例"这个扩展出现之前（Haskell 98之前），前人是怎么hack这个问题的。然后hack前人的方法的这种思维。]
...
包括熟悉的 ``data`` 关键字以外，Haskell 提供我们另外一种方式创建新类型，即采用 ``newtype`` 关键字。
...
``newtype`` 声明的目的是重命名一个存在着的类型，以给它一个独特的身份(id)。正如我们所见，他在表面上是相似的，相对于一个类型，其（该类型）被声明采用 ``data`` 关键字。
...
.. note::
    ...
    作为一个 ``UniqueID`` 的用户，我们只知道它有一个"唯一标识符"(Unique ID,英语字面意思)；我们并不知道它被实现，当作一个 ``Int`` 。

当我们声明一个 ``newtype`` 时，我们必须选择哪个潜在类型的类型类实例，对其（该实例）我们想暴露。
这里，我们决定使 ``NewtypeInt`` 提供 ``Int`` 的实例，为(for) ``Eq`` ， ``Ord`` 和 ``Show`` 。
...
如果 ``newtype`` 没用自动派生来暴露一个类型类的潜在（underlying）类型实现的话，我们是自由的，或者写一个新实例，或者留那个类型类处于不实现状态。
...
在此之上，还有另一个重要的区别于 ``data`` 和 ``newtype`` 之间。一个类型，由 ``data`` 关键字创建，有一个簿记保持（book-keeping）的开销在运行时。例如，追踪（track）哪个构造器，用其（构造器）一个值被创建。而 ``newtype`` 只有一个构造器，所以不需要这个额外开销。
这使得它在运行时更省时间和空间。
